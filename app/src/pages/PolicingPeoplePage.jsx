...
export default function PolicingPeoplePage() {
  const [params] = useSearchParams(); const [search,setSearch] = useState(params.get('q') || '')
  const [rows,setRows] = useState([]); const [form,setForm] = useState(EMPTY); const [showForm,setShowForm] = useState(false); const [busy,setBusy] = useState(false); const [error,setError] = useState('')
  async function load(term=search){ setError(''); try{ setRows(await listPolicePersons(term)) }catch(e){setError(e?.message||'Unable to load persons.')} }
  useEffect(()=>{ load(params.get('q') || '') },[params.get('q')])
  async function submit(e){e.preventDefault();setBusy(true);setError('');try{await createPolicePerson({...form,markers:form.markers.split(',').map(x=>x.trim()).filter(Boolean)});setForm(EMPTY);setShowForm(false);await load()}catch(err){setError(err?.message||'Unable to create person.')}finally{setBusy(false)}}
  return <div className="policing-page page-pad compact-pad"><div className="policing-page-head"><div><span>POLICING · PEOPLE</span><h1>Person search</h1></div><button className="primary-button" onClick={()=>setShowForm(v=>!v)}><Plus size={15}/>New person</button></div>
    ...
    <div className="policing-list">{rows.length===0?<div className="empty-state">No person records found.</div>:rows.map(row=><article key={row.id}><UserRound size={18}/><div><strong>{row.last_name?.toUpperCase()}, {row.first_name}</strong><span>{row.reference} {row.dob?`· DOB ${row.dob}`:''}</span><p>{row.address||'No address recorded.'}</p>{row.markers?.length?<div className="policing-tags">{row.markers.map(m=><b key={m}>{m}</b>)}</div>:null}</div></article>)}</div>
  </div>
}
