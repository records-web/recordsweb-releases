...
function CheckoutReturn() {
  ...
  return (
    <main className="stripe-checkout-shell stripe-checkout-result" style={checkoutPageScrollStyle}>
    ...
    <main className="stripe-checkout-shell" style={checkoutPageScrollStyle}>
      ...
      <div className="stripe-checkout-layout">
        ...
        <form className="stripe-checkout-card stripe-payment-card" onSubmit={submit}>
          ...
          <button type="submit" className="stripe-checkout-primary" disabled={!ready || submitting || Boolean(billing?.billing_payment_exempt)}><LockKeyhole size={15}/>{submitting ? 'Confirming…' : `Subscribe${summary ? ` · ${stripeTotal || moneyPence(summary.dueToday)} today` : ''}`}</button>
