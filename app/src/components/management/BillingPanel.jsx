...
import { createStripeHostedCheckout, createStripePortalSession, getOrganisationBilling } from '../../lib/billingService'
...
export default function BillingPanel() {
  ...
  async function startCheckout() {
    ...
    setError('')
    try {
      const result = await createStripeHostedCheckout()
  ...
  const paymentConnected = Boolean(billing?.stripe_subscription_id) && !['canceled', 'incomplete_expired'].includes(String(billing?.stripe_subscription_status || ''))
  ...
  return (
    <section className="organisation-billing-panel">
      ...
      {billing && <>
          ...
          <article>
            ...
            {!billing.billing_payment_exempt && <div className="billing-action-row">
              {paymentConnected ? (
                ...
                <button type="button" className="primary" onClick={startCheckout} disabled={actionBusy}><CreditCard size={14}/>{actionBusy ? 'Opening…' : 'Set up subscription'}</button>
              )}
            </div>}
          </article>
        ...
        <div className="billing-support-note"><CheckCircle2 size={16}/><span>{billing.billing_payment_exempt ? 'Platform Management has excluded this community from payment. No Stripe checkout is required.' : 'Payments open securely in your browser through Stripe. RecordsWeb does not receive or store your full card number.'} For billing questions, contact <strong>contactus@recordsweb.org</strong>.</span></div>
      </>}
    </section>
  )
}
