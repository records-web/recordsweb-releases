...
The RecordsWeb Web deployment request form saves the request first, then calls `/api/request-confirmation`. The mail endpoint looks up the saved request with the Supabase service role before sending, so it cannot be used as a generic arbitrary-email relay.
