...
After applying the SQL migration, deploy the updated web app and/or build the updated Electron app. The existing Discord Edge Function can continue to be used. In the 3.6.0 source bundle its default RecordsWeb version is 3.6.0.

If you store `RECORDSWEB_VERSION` as a Supabase secret, update it to `3.6.0` before redeploying `recordsweb-discord`.
