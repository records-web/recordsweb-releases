# RecordsWeb 3.3.3 — Typical Dosage & Quantity Calculator
...
## 3.3.3 consultation medication workflow

The Medication section of an open consultation now includes **Add medication**. It opens the same GP medicines prescribing modal used by the main Medication record, including dosage/quantity calculations, specialist-drug controls and prescribing PIN authorisation. Once authorised, the medicine is added immediately to the patient's medication record and summarised in the consultation when it is saved.

For medicines whose supplied GP reference does not contain one exact calculable regimen, RecordsWeb explains why automatic quantity is unavailable instead of presenting the disabled control without context. No dose or quantity is invented where the supplied reference is product-specific, indication-specific or individualised.
