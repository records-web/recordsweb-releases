...
create table if not exists public.recordsweb_access_requests (
  id uuid primary key default gen_random_uuid(), community_name text not null,
  requested_mode text not null default 'general_practice', discord_url text not null,
...
create or replace function public.recordsweb_submit_access_request(p_request_id uuid,p_community_name text,p_requested_mode text,p_discord_url text,p_roblox_group_url text,p_member_range text,p_contact_name text,p_contact_email text,p_discord_username text default null,p_logo_path text default null,p_additional_details text default null,p_authorised_contact boolean default false)
returns uuid language plpgsql security definer set search_path=public as $$
declare v_id uuid:=coalesce(p_request_id,gen_random_uuid()); v_mode text:=lower(trim(coalesce(p_requested_mode,''))); v_members text:=trim(coalesce(p_member_range,'')); v_email text:=lower(trim(coalesce(p_contact_email,'')));
begin
 ...
 if trim(coalesce(p_discord_url,'')) !~* '^https://' or trim(coalesce(p_roblox_group_url,'')) !~* '^https://' then raise exception 'Community URLs must use HTTPS.'; end if;
 ...
 insert into public.recordsweb_access_requests(id,community_name,requested_mode,discord_url,roblox_group_url,member_range,contact_name,contact_email,discord_username,logo_path,additional_details,authorised_contact,status)
 values(v_id,trim(p_community_name),v_mode,trim(p_discord_url),trim(p_roblox_group_url),v_members,trim(p_contact_name),v_email,nullif(trim(coalesce(p_discord_username,'')),''),p_logo_path,nullif(trim(coalesce(p_additional_details,'')),''),true,'pending'); return v_id;
end $$;
...
insert into storage.buckets(id,name,public,file_size_limit,allowed_mime_types) values('recordsweb-access-request-logos','recordsweb-access-request-logos',false,4194304,array['image/png','image/jpeg','image/webp']) on conflict(id) do update set public=excluded.public,file_size_limit=excluded.file_size_limit,allowed_mime_types=excluded.allowed_mime_types;
drop policy if exists "recordsweb_access_request_logo_insert" on storage.objects;
create policy "recordsweb_access_request_logo_insert" on storage.objects for insert to anon,authenticated with check(bucket_id='recordsweb-access-request-logos' and (storage.foldername(name))[1]='requests' and (storage.foldername(name))[2] ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$' and lower(storage.extension(name)) in ('png','jpg','jpeg','webp'));
...
-- Authorised operator identity: gus.farnsworth@XX.XX, where XX.XX matches the
