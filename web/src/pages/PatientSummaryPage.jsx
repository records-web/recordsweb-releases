      ...
      <div className="summary-grid page-pad compact-pad">
        <div className="summary-column">
          ...
          <Panel title="Problems" count={data.problems.length} actions={<button className="link-button" onClick={() => navigate(`/patients/${patientId}/problems`)}>View all</button>}>
            {data.problems.length ? <div className="problems-summary-list">{data.problems.slice(0, 8).map((x, index) => <button type="button" className="problems-summary-row" key={x.id} onClick={() => navigate(`/patients/${patientId}/problems`)}><span className="problems-summary-index">{index < 26 ? String.fromCharCode(65 + index) : index - 25}</span><strong>{x.name}</strong><em>({x.significance || 'Minor'})</em></button>)}</div> : <div className="empty-state">No active problems recorded.</div>}
          </Panel>
