...
export default function PolicingPeoplePage() {
  ...
  return <div className="policing-page page-pad compact-pad">
    ...
    <div className="policing-list">{rows.length === 0 ? <div className="empty-state">No person records found.</div> : rows.map((row) => <article key={row.id}><UserRound size={18}/><div><div className="police-list-title-row"><Link to={`/policing/people/${row.id}`}><strong>{row.last_name?.toUpperCase()}, {row.first_name}</strong></Link><Link className="police-open-link" to={`/policing/people/${row.id}`}>Open record <ChevronRight size={12}/></Link></div><span>{row.reference} {row.dob ? `· DOB ${row.dob}` : ''}</span><p>{row.address || 'No address recorded.'}</p>{row.markers?.length ? <div className="policing-tags">{row.markers.map((m) => <b key={m}>{m}</b>)}</div> : null}</div></article>)}</div>
  </div>
}
