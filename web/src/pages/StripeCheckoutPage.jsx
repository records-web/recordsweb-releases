...
function loadStripeScript() {
  ...
  stripeScriptPromise = new Promise((resolve, reject) => {
    ...
    script.onerror = () =>
      reject(
        new Error(
          "Stripe.js could not be loaded. Check your network connection and Content Security Policy.",
        ),
      );
...
function fallbackSummary(billing) {
  ...
  const first = Math.round(
    Number(billing.billing_first_month_price || 0) * 100,
  );
  ...
  const offer =
    Boolean(billing.billing_first_month_offer) &&
    !billing.billing_first_month_offer_redeemed_at;
  const setupDue = !offer && !billing.billing_setup_fee_paid_at ? setup : 0;
  const boardDue =
    billing.announcement_board_enabled && !billing.announcement_board_paid_at
      ? board
      : 0;
  return {
...
function CheckoutReturn() {
  ...
  useEffect(() => {
    ...
    if (!sessionId) {
      setState({
        loading: false,
        ok: false,
        message: "The Stripe Checkout Session id is missing.",
      });
      return undefined;
    }
      ...
      .catch((error) => {
        if (live)
          setState({
            loading: false,
            ok: false,
            message: error?.message || "Unable to confirm the Stripe checkout.",
          });
      });
    return () => {
      live = false;
  ...
  return (
    <main
      className="stripe-checkout-shell stripe-checkout-result"
      style={checkoutPageScrollStyle}
    >
      ...
      <section className="stripe-checkout-card">
        {state.loading ? (
          <CreditCard className="stripe-checkout-result-icon" size={38} />
        ) : state.ok ? (
          <CheckCircle2
            className="stripe-checkout-result-icon success"
            size={38}
          />
        ) : (
          <TriangleAlert className="stripe-checkout-result-icon" size={38} />
        )}
        <h1>
          {state.loading
            ? "Confirming payment"
...
function StripeCheckoutMain() {
  ...
  const organisationId =
    ...
    session?.profile?.organisations?.id ||
    "";
  const summary = useMemo(
    () => checkoutData?.summary || fallbackSummary(billing),
    ...
    async function initialise() {
      try {
        ...
        if (currentBilling.billing_payment_exempt) {
          setError(
            "This community is excluded from payment. No Stripe checkout is required.",
          );
          return;
        }
        ...
        if (!result?.clientSecret || !result?.publishableKey)
          throw new Error("Stripe checkout could not be initialised.");
        ...
        if (loaded.type !== "success")
          throw new Error(
            loaded?.error?.message ||
              "Stripe checkout actions could not be loaded.",
          );
        actionsRef.current = loaded.actions;
        const stripeSession = loaded.actions.getSession();
        if (stripeSession?.customerDetails?.email)
          setEmail(stripeSession.customerDetails.email);
        ...
        if (live)
          setError(err?.message || "Unable to initialise Stripe checkout.");
      }
    }
    initialise();
    return () => {
        ...
        if (emailResult?.error)
          throw new Error(
            emailResult.error.message ||
              "Stripe could not accept that billing email address.",
          );
      }
      const result = await actionsRef.current.confirm();
      if (result?.type === "error")
        throw new Error(
          result.error?.message || "Stripe could not confirm the subscription.",
        );
    } catch (err) {
  ...
  return (
    <main className="stripe-checkout-shell" style={checkoutPageScrollStyle}>
      <div className="stripe-checkout-brand">
        <img src={recordsWebLogo} alt="RecordsWeb" />
        <div>
          <strong>RecordsWeb</strong>
      ...
      <button
        type="button"
        className="stripe-checkout-back"
        onClick={() => navigate("/management")}
      >
        <ArrowLeft size={14} />
        Back to Management
      ...
      <div className="stripe-checkout-layout">
        <section className="stripe-checkout-card stripe-checkout-summary">
          ...
          <p>
            Complete the payment securely using Stripe. RecordsWeb never
            receives your full card number.
          </p>
          {checkoutData?.environment === "sandbox" && (
            <div className="stripe-sandbox-badge">
              STRIPE SANDBOX · TEST PAYMENT ONLY
            </div>
          )}
          {summary && (
            <div className="stripe-order-summary">
              <div>
                <span>Community</span>
                <strong>
                  {summary.organisationName || "RecordsWeb community"}
                  {summary.organisationCode
                    ? ` · @${summary.organisationCode}`
                    : ""}
                </strong>
              </div>
              <div>
                <span>RecordsWeb Standard</span>
                <strong>{moneyPence(summary.monthlyAmount)} / month</strong>
              </div>
              {summary.firstMonthOffer && (
                <div>
                  <span>First-month offer</span>
                  <strong>{moneyPence(summary.firstMonthAmount)}</strong>
                </div>
              )}
              {summary.setupFeeAmount > 0 && (
                <div>
                  <span>One-off setup fee</span>
                  <strong>{moneyPence(summary.setupFeeAmount)}</strong>
                </div>
              )}
              {summary.setupFeeIncluded && (
                <div>
                  <span>Setup fee</span>
                  <strong>Included in first-month offer</strong>
                </div>
              )}
              {summary.announcementBoardAmount > 0 && (
                <div>
                  <span>Announcement board integration</span>
                  <strong>{moneyPence(summary.announcementBoardAmount)}</strong>
                </div>
              )}
              <div className="stripe-order-total">
                <span>Due today</span>
                <strong>{stripeTotal || moneyPence(summary.dueToday)}</strong>
              </div>
              <small>
                After the introductory month, the recurring subscription is{" "}
                {moneyPence(summary.monthlyAmount)} per month unless cancelled.
              </small>
            </div>
          )}
          <div className="stripe-security-note">
            <ShieldCheck size={16} />
            <span>
              Payment details are collected inside Stripe's secure Payment
              Element.
            </span>
          </div>
        </section>
        ...
        <form
          className="stripe-checkout-card stripe-payment-card"
          onSubmit={submit}
        >
          <header>
            <CreditCard size={20} />
          ...
          <label className="stripe-email-field">
            <span>
              Billing email
              {checkoutData?.emailUpdateRequired === false
                ? " · managed by Stripe"
                : ""}
            </span>
            <input
              ...
              required
              disabled={checkoutData?.emailUpdateRequired === false}
            />
          </label>
          <div className="stripe-payment-element" ref={paymentElementRef}>
            <span>{ready ? "" : "Loading secure payment form…"}</span>
          </div>
          {error && (
          ...
          >
            ...
            {submitting
              ? "Confirming…"
              : `Subscribe${summary ? ` · ${stripeTotal || moneyPence(summary.dueToday)} today` : ""}`}
          </button>
          <small className="stripe-checkout-legal">
            By confirming, you authorise RecordsWeb to charge the recurring
            subscription amount shown above according to the billing schedule.
            You can manage or cancel the subscription through the RecordsWeb
            billing page.
          </small>
        </form>
      </div>
